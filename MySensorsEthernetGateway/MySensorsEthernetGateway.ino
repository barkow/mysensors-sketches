...
void presentation() {
  ...
  present(CHILD_ID_RF, S_INFO);
}

void executeRfCommand(char * command) {
  unsigned int i = 0;
...
void receive(const MyMessage &message) {
  ...
  if ((message.sensor == CHILD_ID_RF) && (message.type == V_TEXT)) {
    Serial.print(F("Received command: "));
    const char* command = message.getString();
    Serial.println(command);
    executeRfCommand((char*)command); 
  }
}
